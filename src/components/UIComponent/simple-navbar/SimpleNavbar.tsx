...
const Navbar = () => {
  ...
  const [mobileServicesCategory, setMobileServicesCategory] = useState<string>("build-secure");
  ...
  const toggleMobileMenu = () => {
    ...
    setMobileServicesCategory("build-secure");
  ...
  return (
    <>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-slate-900 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14 lg:h-16">
                  ...
                  {navItems.map((item) => (
                    ...
                    >
                      {item.href ? (
                        <Link
                          href={item.href}
                          className="text-white hover:text-gray-300 px-2 py-2 text-sm font-medium transition-colors duration-200"
                        >
                          {item.label}
                        </Link>
                      ) : (
                      ...
                      {hoveredItem === item.label && (
                        <div className="absolute bottom-0 left-0 w-full h-0.5 bg-white transition-all duration-300"></div>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className="hidden lg:block">
              <button className="px-5 py-2 rounded-md text-sm font-medium text-white border bg-red-600 border-red-600 transition-all duration-300 ease-in-out hover:bg-transparent hover:text-red-600">
                Contact
              </button>
            </div>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="relative flex flex-col h-full bg-white">
            {/* Header with logo and close button - Red Background - Match navbar height */}
            <div className="flex items-center justify-between px-4 py-3 bg-red-600 h-14">
            ...
            <div className="p-4 bg-white border-t border-gray-100">
              <button 
                className="w-full px-6 py-3 rounded-md text-base font-medium text-red-600 border border-red-600 transition-all duration-300 ease-in-out hover:bg-red-600 hover:text-white"
                onClick={handleMobileLinkClick}
              >
                Contact
      ...
      {/* Desktop Overlay */}
      {hoveredItem && (
        <div className="fixed left-0 top-0 w-full h-screen opacity-100 visible z-30 bg-black bg-opacity-50 transition-all duration-300 hidden lg:block" />
      )}
    </>
  );
};
